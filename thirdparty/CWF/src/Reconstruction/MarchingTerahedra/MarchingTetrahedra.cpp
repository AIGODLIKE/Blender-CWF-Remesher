...
_Marching_Tetrahedra::_Marching_Tetrahedra(const std::pair<_Point3, _Point3> &in_boundingbox, const int &in_depth) : _boundingbox(in_boundingbox), _depth(in_depth), _method(0) { tiling_(); }
...
void _Marching_Tetrahedra::tiling_() {
  ...
  _Point3 lu(-box * 0.25 * (1.0 / sqrt(3.0) + 1.0), box * 0.25 * (1.0 + sqrt(3.0)), _boundingbox.first.z() - 0.1 * (_boundingbox.second.z() - _boundingbox.first.z()) - 3 * a);
  ...
  int height = (int)(((_boundingbox.second.z() + 0.1 * (_boundingbox.second.z() - _boundingbox.first.z())) - lu.z() - 2.0 * a) / (3.0 * a)) + 1;
  ...
  for (int i = 0; i < row; ++i) {
    for (int j = 0; j < row + i; ++j) {
      ...
      for (int h = 0; h < height; ++h) {
        ...
        _tetras.push_back(std::make_tuple(from_sym_to_vertex[v0], from_sym_to_vertex[v2], from_sym_to_vertex[v1], from_sym_to_vertex[v3]));
      ...
      for (int h = 0; h < height; ++h) {
        ...
        _tetras.push_back(std::make_tuple(from_sym_to_vertex[v0], from_sym_to_vertex[v1], from_sym_to_vertex[v2], from_sym_to_vertex[v3]));
        v0 = std::make_tuple(p1.first, p1.second, h * 3 + 1);
        v1 = std::make_tuple(p1.first, p1.second, h * 3 + 4);
        v2 = std::make_tuple(p2.first, p2.second, h * 3 + 2);
        v3 = std::make_tuple(p0.first, p0.second, h * 3 + 3);
