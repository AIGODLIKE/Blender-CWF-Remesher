...
bool _Polygon::is_in_(const _Point2 &in_p) const {
  ...
  for (int i = 0; i < _points.size(); ++i) {
    ...
    if (_BOC::sign_((__p1 - in_p).cross_(__p2 - in_p).length_()) == _BOC::_Sign::ZerO &&
        (_BOC::sign_((__p1 - in_p).dot_((__p2 - in_p))) == _BOC::_Sign::NegativE || _BOC::sign_((__p1 - in_p).dot_((__p2 - in_p))) == _BOC::_Sign::ZerO)) {
      return false;
    }
