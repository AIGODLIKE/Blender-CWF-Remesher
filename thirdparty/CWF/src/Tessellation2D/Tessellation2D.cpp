...
namespace BGAL
{
  ...
  const std::vector<std::vector<std::array<int, 2>>> &_Tessellation2D::get_cells_() const
  {
    return _cells;
  }
  ...
  void _Tessellation2D::calculate_()
  {
    ...
    for (int i = 0; i < _num_sites; ++i)
    {
      ...
      do
      {
        ...
        {
          _cells[i].push_back({from_sym_to_id[_p], _adj});
        }
        else
        {
          from_sym_to_id[_p] = _tessellation_vertices.size();
          _cells[i].push_back({(int)_tessellation_vertices.size(), _adj});
          _tessellation_vertices.push_back(convert_sym_to_point_(_p));
        }
        _cell_polygons[i].insert_(_tessellation_vertices[_cells[i].back()[1]]);
