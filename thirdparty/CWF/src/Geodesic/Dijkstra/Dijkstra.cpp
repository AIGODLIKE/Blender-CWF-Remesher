...
void _Dijkstra::implement_() {
  struct _Event : std::tuple<double, int, int, int, int> {
    _Event(double dis, int self, int parent, int root, int level) : std::tuple<double, int, int, int, int>(dis, self, parent, root, level) {}

    bool operator<(const _Event &right) const { return std::get<0>(*this) < std::get<0>(right); }
...
_Dijkstra::_Dijkstra(const _ManifoldModel &in_model, const std::map<int, double> &in_sources) : _Abstract_Method(in_model, in_sources) { _method = 1; }
