...
_Model::_MFace::_MFace(const int &id1, const int &id2, const int &id3, const _Point3 &p1, const _Point3 &p2, const _Point3 &p3) : _Triangle3(p1, p2, p3), id(-1) {
  _vertices.resize(3);
  _vertices[0] = id1;
  _vertices[1] = id2;
  _vertices[2] = id3;
...
_Vertex_Iterator _Model::vertex_begin() const { return _Vertex_Iterator(this, 0); }
...
void _Model::save_obj_file_(const std::string &in_file_name) const {
  ...
  for (int i = 0; i < _vertices.size(); ++i) {
    out << "v " << std::setiosflags(std::ios::fixed) << std::setprecision(20) << _vertices[i].x() << " " << _vertices[i].y() << " " << _vertices[i].z() << std::endl;
  }
...
void _Model::save_scalar_field_obj_file_(const std::string &in_file_name, const std::vector<double> &vals) const {
  ...
  for (int i = 0; i < number_faces_(); ++i) {
    ...
    out << "f " << _faces[i][0] + 1 << "/" << _faces[i][0] + 1 << " " << _faces[i][1] + 1 << "/" << _faces[i][1] + 1 << " " << _faces[i][2] + 1 << "/" << _faces[i][2] + 1 << std::endl;
  }
  out.close();
}

void _Model::save_pamametrization_obj_file_(const std::string &in_file_name, const std::vector<std::pair<double, double>> &uvs) const {
  std::ofstream out(in_file_name);
  int folder_loc = in_file_name.rfind("\\") > in_file_name.rfind("/") ? in_file_name.rfind("\\") : in_file_name.rfind("/");
  int dot_loc = in_file_name.rfind('.');
  out << "g " << in_file_name.substr(folder_loc + 1, dot_loc - folder_loc - 1) << std::endl;
  ...
  for (int i = 0; i < uvs.size(); ++i) {
    out << "vt " << std::setiosflags(std::ios::fixed) << std::setprecision(20) << uvs[i].first << " " << uvs[i].second << std::endl;
  }
  for (int i = 0; i < number_faces_(); ++i) {
    if (_faces_useless.find(i) != _faces_useless.end()) {
      continue;
...
void _Model::compute_normal_boundingbox_() {
  ...
  for (int i = 0; i < _faces.size(); ++i) {
    _Point3 norm = (_vertices[_faces[i][1]] - _vertices[_faces[i][0]]).cross_((_vertices[_faces[i][2]] - _vertices[_faces[i][0]]));
...
_Model::_PQP_Query_Resutl::_PQP_Query_Resutl(const int &in_pos_flag, const int &in_triangle_id, const double &in_distance, const _Point3 &in_nearset_point)
