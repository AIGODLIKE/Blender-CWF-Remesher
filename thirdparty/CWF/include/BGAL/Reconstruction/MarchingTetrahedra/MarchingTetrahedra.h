...
template <class F> _ManifoldModel _Marching_Tetrahedra::reconstruction_(const F &sign) {
      ...
      for (int j = 0; j < 3; ++j) {
        ...
        if (from_edge_to_vertex.find(std::make_pair(v0, v1)) == from_edge_to_vertex.end()) {
          ...
          case 1: {
            ...
            _vertices.push_back(_tetra_vertices[v0] * fabs(signs[v1]) / len + _tetra_vertices[v1] * fabs(signs[v0] / len));
            break;
          }
          default:
            break;
