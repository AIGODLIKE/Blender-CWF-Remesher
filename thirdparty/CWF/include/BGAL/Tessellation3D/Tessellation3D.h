...
private:
  ...
  std::vector<std::vector<std::array<int, 3>>> _cells;
  std::vector<std::map<int, std::vector<std::array<int, 2>>>> _edges;
...
private:
  struct _Symbolic_Point {
    ...
    _Symbolic_Point(const std::set<int> &in_sym, const int &in_site) : _sym(in_sym), _site(in_site), flag(0) { p.clear(); }

    _Symbolic_Point(const int &in1, const int &in2, const int &in3, const int &in_site) : _site(in_site), flag(0) {
      _sym.clear();
      _sym.insert(in1);
      _sym.insert(in2);
      _sym.insert(in3);
    ...
    /// 查找包含: 返回所有 _sym 和 in_sym 都包含的元素
  ...
  _Symbolic_Point insec_bisector_(const _Symbolic_Point &p1, const _Symbolic_Point &p2, const int &neigh, const int &center, const std::vector<int> &current_face_vid) const;

  void calculate_();

public:
  _Restricted_Tessellation3D(const _ManifoldModel &in_model);
  _Restricted_Tessellation3D(const _ManifoldModel &in_model, const std::vector<_Point3> &in_sites, const std::vector<double> &in_weights);
  ...
  const std::vector<std::vector<std::array<int, 3>>> &get_cells_() const { return _cells; }

  const std::vector<std::map<int, std::vector<std::array<int, 2>>>> &get_edges_() const { return _edges; }
};
} // namespace BGAL