...
public:
  ...
  _Tessellation2D(const _Polygon &in_boundary, const std::vector<_Point2> &in_sites, const std::vector<double> &in_weights);
  void calculate_(const _Polygon &in_boundary, const std::vector<_Point2> &in_sites);
  void calculate_(const _Polygon &in_boundary, const std::vector<_Point2> &in_sites, const std::vector<double> &in_weights);
  const std::vector<_Polygon> &get_cell_polygons_() const;
  const std::vector<std::vector<std::array<int, 2>>> &get_cells_() const;
...
private:
  void calculate_();
  _BOC::_Sign symbolic_point_site_(const int &site1, const int &site2, const std::pair<int, int> &symp, const int &cur_edge);
...
private:
  ...
  std::vector<std::vector<std::array<int, 2>>> _cells;
