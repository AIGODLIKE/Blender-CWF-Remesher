...
void Tessellation3DTest()
{
	...
	for (int i = 0; i < cells.size(); ++i)
	{
		...
		{
			out << "f " << cells[i][j][0] + 1 << "/" << i + 1
				<< " " << cells[i][j][1] + 1 << "/" << i + 1
				<< " " << cells[i][j][2] + 1 << "/" << i + 1 << std::endl;
		}
	}
	//for (int i = 0; i < cells.size(); ++i)
	//{
...
void CPDTest()
{
	...
	const auto &cells = RPD.get_cells_();
	...
	for (int i = 0; i < num; ++i)
	{
		...
		for (int j = 0; j < cells[i].size(); ++j)
		{
			BGAL::_Triangle3 tri(RPD.vertex_(cells[i][j][0]), RPD.vertex_(cells[i][j][1]), RPD.vertex_(cells[i][j][2]));
			cal_mass += tri.area_();
		}
